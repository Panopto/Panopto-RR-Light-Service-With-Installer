...
namespace RRLightProgram
{
    public class StateMachine : IStateMachine
    {
        ...
        /// <summary>
        /// Flash Green to indicate that action is required to accept the recording
        /// </summary>
        private bool ActionRequestPotentialRecording(Input input) // difference between respond and request is that respond is through the tray and request is through the button?
        {
            bool requestResult = false;
            if (!this.remoteRecorder.SupportsStartNewRecording)
            {
        ...
        /// <summary>
        /// Turn flashing green light on for a potential recording
        /// </summary>
        ...
        /// <summary>
        /// For explicit display in logs of button down in recording, no change in state.
        /// </summary>
        private bool ActionRespondButtonDownInRecording(Input input)
        {
            this.light.SetSolid(LightColor.Green);
            return true;
        }
        ...
        /// <summary>
        /// For explicit display in logs of button up in recording, no change in state.
        /// </summary>
        private bool ActionRespondButtonUpInRecording(Input input)
        {
            this.light.SetSolid(LightColor.Green);
            return true;
        }
        ...
        /// <summary>
        /// For explicit display in logs of button down in paused, no change in state.
        /// </summary>
        private bool ActionRespondButtonDownInPaused(Input input)
        {
            this.light.SetSolid(LightColor.Yellow);
            return true;
        }
        ...
        /// For explicit display in logs of button down in paused, no change in state.
        /// </summary>
        private bool ActionRespondButtonUpInPaused(Input input)
        {
            this.light.SetSolid(LightColor.Yellow);
            return true;
        }
        ...
        /// For explicit display in logs of button down in paused, no change in state.
        /// </summary>
        private bool ActionRespondButtonDownInStopped(Input input)
        {
            this.light.SetSolid(LightColor.Off);
            return true;
        }
        ...
        /// For explicit display in logs of button down in paused, no change in state.
        /// </summary>
        private bool ActionRespondButtonUpInStopped(Input input)
        {
            this.light.SetSolid(LightColor.Off);
            return true;
        }
        ...
        /// For explicit display in logs of button down in paused, no change in state.
        /// </summary>
        private bool ActionRespondButtonDownInPotentialRecording(Input input)
        {
            this.light.SetSolid(LightColor.Green);
            return true;
        }
        ...
        /// For explicit display in logs of button down in paused, no change in state.
        /// </summary>
        private bool ActionRespondButtonUpInPotentialRecording(Input input)
        {
            this.light.SetSolid(LightColor.Green);
            return true;
        }
        ...
        /// </summary>
        private bool ActionRespondButtonDownInPreviewing(Input input)
        {
            if (!this.remoteRecorder.SupportsStartNewRecording)
            {
                this.light.SetSolid(LightColor.Red);
        ...
        /// Turn light off.
        /// </summary>
        private bool ActionRespondButtonUpInPreviewing(Input input)
        {
            if (!this.remoteRecorder.SupportsStartNewRecording)
            {
                this.light.SetSolid(LightColor.Off);
        ...
        private bool ActionRequestStopAndDelete(Input input)
        {
            ...
            if (!requestResult)
            {
                ...
                else if (currentState == State.PotentialRecording)
                {
                    this.light.SetFlash(LightColor.Green);
                }
                else
            ...
            Console.Write("Using id to delete recording...\n");
            return requestResult;
        }

        private bool ActionRespondDeletePreviousRecording(Input input)
        {
            /*State currentState = this.state;
            bool requestResult = this.remoteRecorder.StopCurrentRecording();
            if (!requestResult)
        ...
        private void InitializeTransitionTable()
        {
            ...
            this.transitionTable.Add(new Condition(State.PreviewingNoNextSchedule,   Input.ButtonDown),                         new Transition(this.ActionRespondButtonDownInPreviewing,    State.PreviewingNoNextSchedule));
            this.transitionTable.Add(new Condition(State.PreviewingNoNextSchedule,   Input.ButtonUp),                           new Transition(this.ActionRespondButtonUpInPreviewing,      State.PreviewingNoNextSchedule));
            this.transitionTable.Add(new Condition(State.PreviewingNoNextSchedule,   Input.ButtonPressed),                      new Transition(this.ActionRequestPotentialRecording,        State.PotentialRecording));
            ...
            // difference between state.transitionanytorecoring and state.recording?
            ...
            this.transitionTable.Add(new Condition(State.PreviewingWithNextSchedule, Input.ButtonDown),                         new Transition(this.ActionRespondButtonDownInPreviewing,    State.PreviewingWithNextSchedule));
            this.transitionTable.Add(new Condition(State.PreviewingWithNextSchedule, Input.ButtonUp) ,                          new Transition(this.ActionRespondButtonUpInPreviewing,      State.PreviewingWithNextSchedule));
            ...
            this.transitionTable.Add(new Condition(State.PotentialRecording, Input.RecorderPreviewingNoNextSchedule),   new Transition(this.ActionRespondPreviewing,                     State.PreviewingNoNextSchedule)); // TODO: do we need to clean up potential recordings that end of their own accord?  Why wouldn't we transition to stopped first?
            this.transitionTable.Add(new Condition(State.PotentialRecording, Input.RecorderPreviewingWithNextSchedule), new Transition(this.ActionRespondPreviewing,                     State.PreviewingWithNextSchedule)); // TODO: do we need to clean up potential recordings that end of their own accord?  Why wouldn't we transition to stopped first?
            ...
            this.transitionTable.Add(new Condition(State.PotentialRecording, Input.RecorderStopped),                    new Transition(this.ActionRespondDeletePreviousRecording,        State.Stopped));
            this.transitionTable.Add(new Condition(State.PotentialRecording, Input.RecorderDormant),                    new Transition(this.ActionRespondDeletePreviousRecording,        State.Dormant));
            ...
            this.transitionTable.Add(new Condition(State.PotentialRecording, Input.ButtonDown),                         new Transition(this.ActionRespondButtonDownInPotentialRecording, State.PotentialRecording));
            this.transitionTable.Add(new Condition(State.PotentialRecording, Input.ButtonUp),                           new Transition(this.ActionRespondButtonUpInPotentialRecording,   State.PotentialRecording));
            this.transitionTable.Add(new Condition(State.PotentialRecording, Input.ButtonPressed),                      new Transition(this.ActionRespondRecording,                      State.Recording));
            this.transitionTable.Add(new Condition(State.PotentialRecording, Input.ButtonHeld),                         new Transition(this.ActionRequestStopAndDelete,                  State.TransitionRecordingToStop));
            ...
            this.transitionTable.Add(new Condition(State.Recording, Input.ButtonDown),                         new Transition(this.ActionRespondButtonDownInRecording, State.Recording));
            this.transitionTable.Add(new Condition(State.Recording, Input.ButtonUp),                           new Transition(this.ActionRespondButtonUpInRecording,   State.Recording));
            ...
            this.transitionTable.Add(new Condition(State.Paused, Input.ButtonDown),                         new Transition(this.ActionRespondButtonDownInPaused,    State.Paused));
            this.transitionTable.Add(new Condition(State.Paused, Input.ButtonUp),                           new Transition(this.ActionRespondButtonUpInPaused,      State.Paused));
            ...
            this.transitionTable.Add(new Condition(State.Stopped, Input.ButtonDown),                            new Transition(this.ActionRespondButtonDownInStopped,   State.Stopped));
            this.transitionTable.Add(new Condition(State.Stopped, Input.ButtonUp),                              new Transition(this.ActionRespondButtonUpInStopped,     State.Stopped));
            this.transitionTable.Add(new Condition(State.Stopped, Input.ButtonPressed),                         new Transition(this.ActionRequestPotentialRecording,    State.PotentialRecording)); // test code, simulates the remote recorder's preview with schedule option
            this.transitionTable.Add(new Condition(State.Stopped, Input.ButtonHeld),                            new Transition(this.ActionRequestNewRecording,          State.Recording));          
